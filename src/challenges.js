...
function howManyTimes(list, repeatedWord) {
	return list.filter(word => word === repeatedWord).length;
}
...
function filterOut(originalItems, itemsToFilterOut) {
	if (!originalItems.length) return null;
	return originalItems.filter(item => !itemsToFilterOut.includes(item));
}
